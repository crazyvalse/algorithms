var Vue = (function (exports) {
  ...
  */
    ...
    if (setup) {
      ...
      if (isPromise(setupResult)) {
        ...
        if (isSSR) {
          ...
          // bail here and wait for re-retry.
  ...
  function buildProps(node, context, props = node.props, ssr = false) {
    ...
    for (let i = 0; i < props.length; i++) {
      ...
      if (prop.type === 6 /* ATTRIBUTE */) {
        ...
        properties.push(
          createObjectProperty(createSimpleExpression(name, true, getInnerRange(loc, 0, name.length)), valueNode)
        )
      } else {
        // directives
  ...
  // This retry is the "full-build" that includes both the runtime
  {
    initDev()
  }
